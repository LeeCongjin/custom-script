...
export async function genModal() {
	...
	try {
		...
		const TemplateModalData = await reWriteTemplate(
			path.join(templateModalDirPath, "TemplateModal.vue"),
			TemplateModalChangeMap(modalName)
		)
		await fse.outputFile(
			path.join(
				outputDirPath,
		...
		const modalProviderData = await reWriteTemplate(
			...
			modalProviderChangeMap(
				modalName,
				"./" + path.relative(path.dirname(modalProviderPath), outputDirPath)
			)
		)
