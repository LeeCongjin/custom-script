...
import ora from "ora";
import { modalProviderChangeMap, useModalChangeMap, } from "./changMap.js";
...
export async function genModal() {
    ...
    try {
        ...
        await fse.copy(path.join(templateModalDirPath, "TemplateModal.vue"), path.join(outputDirPath, `${firstWordUpper(stringToHump(modalName))}Modal.vue`));
        const modalProviderData = await reWriteTemplate(modalProviderPath, modalProviderChangeMap(modalName, outputDirPath));
